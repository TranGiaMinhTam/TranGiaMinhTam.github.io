...
describe('U-04 research styles', () => {
  it('uses local responsive geometry, focus inheritance, and reduced-motion support', () => {
    ...
    expect(styles).toMatch(/\.signalSheet\s*\{[^}]*grid-column:\s*1[^}]*min-block-size:\s*26rem/s)
    expect(styles).toMatch(/\.analysisContext\s*\{[^}]*grid-column:\s*2[^}]*min-block-size:\s*26rem/s)
    expect(styles).toMatch(/\.innovationPreview\s*\{[^}]*grid-column:\s*1 \/ -1/s)
    expect(styles).toMatch(/\.relationshipSummary\s*\{[^}]*grid-column:\s*1 \/ -1/s)
